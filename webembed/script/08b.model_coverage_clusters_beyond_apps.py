...
FEATURES = ['doc2vec-distance-tags', 'doc2vec-distance-content', 'doc2vec-distance-content_tags', 'doc2vec-distance-all',
            'dom-rted', 'visual-hyst', 'visual-pdiff']
    ...
    for app in APPS:
        ...
        for feature in FEATURES:
            ...
            for classifier in CLASSIFIERS:
                ...
                if feature == 'dom-rted':
                    feature = 'dom-rted'.upper()
                elif feature == 'visual-hyst':
                    feature = 'VISUAL_Hyst'
                elif feature == 'visual-pdiff':
                    feature = 'VISUAL-PDiff'

                ss = pd.read_csv('script/SS_threshold_set.csv',
                                 usecols=['appname', 'state1', 'state2', feature.replace('-', '_')])
                ...
                EXT = '.sav'
                if feature == 'VISUAL-PDiff':
                    CLASSIFIER_USED = CLASSIFIER_PATH + SETTING + CLASSIFIER + feature.replace('_', '-').lower() + EXT
                else:
                ...
                ss[feature.replace('-', '_')] = ss[feature.replace('-', '_')].map(lambda dist: is_clone(model, dist))
                ...
                print("number of pairs %s: %d" % (feature, number_d2v))
